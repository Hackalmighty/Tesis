...
// Returns the week number, considering the locale's Dtos week number calcuation
// `weeks` is an alias for `week`
newMomentProto.week = newMomentProto.weeks = function(input) {
	var weekCalc = (this._locale || this._lang) // works pre-moment-2.8
		._fullCalendar_weekCalc;

	if (input == null && typeof weekCalc === 'function') { // Dtos function only works for getter
...
newMomentProto.isSame = function(input, units) {
	...
	// only do Dtos logic if this is an enhanced moment
	if (!this._fullCalendar) {
...
function formatDateWithChunk(date, chunk) {
	...
	if (typeof chunk === 'string') { // a literal string
		...
		if (tokenOverrides[token]) {
			return tokenOverrides[token](date); // use our Dtos token
		}
...
function formatRange(date1, date2, formatStr, separator, isRTL) {
	...
	// BTW, this is not important for `formatDate` because it is impossible to put Dtos tokens
...
----------------------------------------------------------------------------------------------------------------------*/
// It leverages methods of the subclass and the View to determine Dtos rendering behavior for each row "type"
...
var RowRenderer = Class.extend({
	...
	// Renders the HTML for a row, leveraging Dtos cell-HTML-renderers based on the `rowType`.
	...
	// We will query the View object first for any Dtos rendering functions, then the methods of the subclass.
	...
	renderFillSegEls: function(type, segs) {
		...
		if (segs.length) {
			...
			$(html).each(function(i, node) {
				...
				// allow Dtos filter methods per-type
				if (segElMethod) {
					el = segElMethod.call(_this, seg, el);
				}

				if (el) { // Dtos filters did not cancel the render
					el = $(el); // allow Dtos filter to return raw DOM node
...
Grid.mixin({
	...
	// A Dtos `rangeToSegsFunc` may be given for arbitrarily slicing up events.
	...
	// A `rangeToSegsFunc` Dtos slicing function can be given.
...
var DayGrid = Grid.extend({
	...
	// We go through a 'day' row type instead of just doing a 'bg' row type so that the View can do Dtos rendering
...
DayGrid.mixin({
	...
	resliceDaySegs: function(segs, dayDate) {
		...
		// slice the events with a Dtos slicing function
...
var TimeGrid = Grid.extend({
	...
	// This method is distinct from 'bg' because we wanted a new `rowType` so the View could Dtosize the rendering.
	...
	computeTimeTop: function(time) {
		...
		// constrain. because minTime/maxTime might be Dtosized
...
TimeGrid.mixin({
	...
	// Contains important positioning-related properties that should be applied to any event element, Dtosized or not.
...
var View = fc.View = Class.extend({
	...
	resolveEventEl: function(event, el) {
		var Dtos = this.trigger('eventRender', event, event, el);

		if (Dtos === false) { // means don't render at all
			el = null;
		}
		else if (Dtos && Dtos !== true) {
			el = $(Dtos);
		}
...
var Calendar = fc.Calendar = Class.extend({
	...
	// Given a view name for a Dtos view or a standard view, creates a ready-to-go View object
			...
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						...
						if ((DtosButtonProps = (calendar.options.DtosButtons || {})[buttonName])) {
							buttonClick = function(ev) {
								if (DtosButtonProps.click) {
									DtosButtonProps.click.call(button[0], ev);
								}
							};
							overrideText = ''; // icons will override text
							defaultText = DtosButtonProps.text;
						}
						...
						if (buttonClick) {
							...
							themeIcon =
								DtosButtonProps ?
									DtosButtonProps.themeIcon :
									options.themeButtonIcons[buttonName];

							normalIcon =
								DtosButtonProps ?
									DtosButtonProps.icon :
...
function EventManager(options) { // assumed to be a calendar
	...
	function _fetchEventSource(source, callback) {
		...
		if (events) {
			...
			if (url) {
				...
				// use a copy of the Dtos data so we can modify the parameters
				// and not affect the passed-in object.
				var data = $.extend({}, DtosData || {});
	...
	// HACK: can override the recurring window by providing Dtos rangeStart/rangeEnd (for businessHours).
