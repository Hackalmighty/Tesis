...
" + Environment.StackTrace;
         ...
         * the CallContext flows not just through async points but also through app Dominio 
         * boundaries. I.e. if you make a remoting call into another app Dominio, the
         * CallContext will flow through this call (which will require all the values it
         * stores to get serialized) and get restored in the other app Dominio.
         * 
         * In our case, our DbContextScope instances aren't serializable. And in any case,
         * we most definitely don't want them to be flown accross app Dominios. So we'll
         ...
         * 
         * That way, if an app Dominio boundary is crossed, the keys will be flown accross
         * but not the DbContextScope instances since a static variable is stored at the 
         * app Dominio level. The code executing in the other app Dominio won't see the ambient
         * DbContextScope created in the first app Dominio and will therefore be able to create
        ...
        internal static DbContextScope GetAmbientScope()
        {
            ...
            if (instanceIdentifier == null)
                return null; // Either no ambient context has been set or we've crossed an app Dominio boundary and have (intentionally) lost the ambient context
